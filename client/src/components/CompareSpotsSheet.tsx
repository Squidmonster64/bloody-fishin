...
export function CompareSpotsSheet({ baseData, savedSpots, onClose }: { baseData: AppData; savedSpots: MySpot[]; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end bg-black/65 sm:items-center sm:justify-center sm:p-4" role="dialog" aria-modal="true" aria-label="Compare fishing spots">
    <div className="max-h-[88svh] w-full overflow-y-auto rounded-t-2xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-2xl sm:max-w-2xl sm:rounded-xl">
      <div className="flex items-start justify-between gap-3 border-b border-[var(--border)] pb-3"><div><h2 className="text-lg font-bold text-[var(--action)]">⚖️ Compare spots</h2><p className="mt-1 text-xs text-[var(--text-muted)]">Stacked day cards are sized for an iPhone SE—no side-scrolling table.</p></div><button onClick={onClose} className="flex min-h-[44px] min-w-[44px] items-center justify-center text-xl text-[var(--text-muted)] hover:text-[var(--text)]" aria-label="Close comparison">×</button></div>
      <div className="mt-4 rounded-lg border border-[var(--border)] bg-[var(--app-bg)] p-3"><label className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)]">Compare {baseData.location.name} with</label><div className="flex gap-2"><select value={selectedKey} onChange={e => setSelectedKey(e.target.value)} className="min-h-[48px] min-w-0 flex-1 rounded border border-[var(--border)] bg-[var(--surface)] px-3 text-base font-semibold text-[var(--text)]"><option value="">Choose a spot…</option>{candidates.map(loc => <option key={`${loc.lat},${loc.lon}`} value={`${loc.lat},${loc.lon}`}>{loc.name}</option>)}</select><button onClick={loadCompare} disabled={!selected || loading} className="min-h-[48px] rounded bg-[var(--action)] px-4 text-sm font-bold text-white disabled:opacity-50">{loading ? "Loading" : "Compare"}</button></div></div>
      {error && <p className="mt-3 rounded border border-red-400/40 bg-red-400/10 p-3 text-xs text-red-200">{error}</p>}
      {compareData && <div className="mt-4 space-y-3">{baseData.daily.map((baseDay, index) => { const other = compareData.daily[index]; if (!other) return null; const baseWins = dailyScore(baseDay) >= dailyScore(other); const date = new Date(`${baseDay.date}T12:00:00`).toLocaleDateString("en-AU", { weekday:"short", day:"numeric", month:"short" }); return <section key={baseDay.date}><p className="mb-1 px-1 text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">{date}</p><div className="grid gap-2"><SpotLine name={baseData.location.name} day={baseDay} winner={baseWins} /><SpotLine name={compareData.location.name} day={other} winner={!baseWins} /></div></section>; })}</div>}
      {!loading && !compareData && !error && <p className="py-8 text-center text-sm text-[var(--text-muted)]">Choose a comparison spot to load its forecast.</p>}
    </div>
  </div>;
}
