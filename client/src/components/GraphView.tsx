/**
 * GraphView — Chart.js multi-line chart with zoom/pan plugin.
 * Pinch to zoom, drag to pan, double-tap/click to reset.
 */
...
export function GraphView({ data, hourlyDay, onDayChange, vis, onToggleVis }: Props) {
  ...
  useEffect(() => {
    if (!canvasRef.current || !dayRows.length) return;
    if (chartRef.current) { chartRef.current.destroy(); chartRef.current = null; }

    const labels = dayRows.map(r => r.hourLabel);
    const datasets = [];

    if (vis.wind) datasets.push({
      label: "Wind (kt)", yAxisID: "y",
      data: dayRows.map(r => r.windKt),
      borderColor: "#7eb8f7", backgroundColor: "rgba(126,184,247,0.08)",
      borderWidth: 2, pointRadius: 0, tension: 0.3, fill: false,
    });
    if (vis.swell) datasets.push({
      label: "Swell (m)", yAxisID: "y2",
      data: dayRows.map(r => r.swellH),
      borderColor: "#3ecf8e", backgroundColor: "rgba(62,207,142,0.08)",
      borderWidth: 2, pointRadius: 0, tension: 0.3, fill: false,
    });
    if (vis.fish) datasets.push({
      label: "Fish %", yAxisID: "y3",
      data: dayRows.map(r => r.fishScore),
    ...
    chartRef.current = new Chart(canvasRef.current, {
      ...
      options: {
        ...
        onHover: (_e, elements) => {
          if (elements.length) setActiveHour(elements[0].index);
        },
        plugins: {
        ...
        scales: {
          x: {
            ticks: { color: "#7a9bb5", font: { size: 10 }, maxRotation: 0 },
  ...
  const activeRow = activeHour != null ? dayRows[activeHour] : null;

  return (
    <div className="flex flex-col h-full overflow-y-auto">
      {/* Day selector */}
      <div className="flex items-center gap-2 px-3 py-2 bg-[#0d1f3c] border-b border-[#1e3a5f] overflow-x-auto scrollbar-hide">
        <span className="text-[10px] text-[#7a9bb5] uppercase tracking-wider whitespace-nowrap flex-shrink-0">📅 Day:</span>
        {data.daily.map(d => {
          const dt = new Date(d.date + "T12:00:00");
          const label = dt.toLocaleDateString("en-AU", { weekday: "short", day: "numeric", month: "short" });
          const isSelected = d.date === hourlyDay;
          const isGolden = d.isGolden;
          return (
            <button key={d.date} onClick={() => onDayChange(d.date)}
              className={`flex-shrink-0 px-2.5 py-1 rounded text-xs font-semibold transition-all duration-150 min-h-[36px] whitespace-nowrap
                ${isSelected ? "bg-[#ff6b35] text-white" : isGolden ? "bg-[#3ecf8e]/20 text-[#3ecf8e] border border-[#3ecf8e]/40 hover:bg-[#3ecf8e]/30" : "bg-[#1e3a5f] text-[#7a9bb5] hover:text-white hover:bg-[#2a4f7a]"}`}>
              {isGolden && !isSelected ? "⭐ " : ""}{label}
            </button>
          );
        })}
      </div>
      ...
      {/* Chart */}
      <div className="relative px-2 py-2" style={{ height: "280px", minHeight: "220px" }}>
        <canvas ref={canvasRef} />
        <p className="absolute bottom-3 right-4 text-[9px] text-[#3a5a7a]">Pinch/scroll to zoom · Drag to pan · Double-tap to reset</p>
      </div>
